<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<script>
$(function(){ // 결제 버튼 2개? -> 반응형이라 두개 생김 

	$("#buy_btn").click(function(){ //모바일용 구매하기 버튼
		if(confirm("구매 진행 하시겠습니까?")){  			//로그인 걸고 세션값으로 걸러주기
		}
	});
 
	
	$("#buy_btn2").click(function(){ // pc용 구매하기 버튼 
		if(confirm("비회원으로 구매 진행 하시겠습니까?")){
			frm.action = "order";
			frm.method = "POST";
			frm.submit();
		}
	...
	$("#buy_basket").click(function(){ // 웹용 구매하기 버튼 
		alert("준비중입니다. ");
	});
	...
	$("#product_ea").change(function(){
		var $ori = $("#price").val();  //price 값 가져오기
		var $ea = $(this).val();
		var $total = $ori * $ea;
		$("#total_goods_price").text($total.toLocaleString());       //inner-js 형태, text-jquery
		$("#total").val();   // 결제시스템에, 들어가면 안됨 
...
<!--앞단에 파라미터 붙어서 와야함 원래는 .. -->

<input type = "hidden" name="product_nm" value="아우라 소파">
<input type = "hidden" name="product_dtc" value="소파">

<input type = "hidden" name="price" id="price" value="111000"> <!-- 절대 변하지 않는값 -->
<input type = "hidden" name="point" id="price" value="11100">
<input type = "hidden" name="total" id="total" value="111000"> <!-- 기본값으로 그대로 넘거아야함.  -->
			...
			<div id="goodsBuyOpenSection" class="goods_buy_open_sections">
				...
				<ul class="goods_buttons_section">
					<li>
						...
						<li><button type="button" id="buy_btn" class="btn_resp size_c color2"><span designElement="text" textIndex="99" >구매하기</span></button></li> <!--모바일용 구매-->
						</ul>
					</li>
...
<div id="select_option_lay">
		...
		<!-- 선택된 옵션 노출 영역 끝 -->
				...
				<div class="goods_price_area">
					...
					<tr>
						...
						<td class="total_goods_price">
							...
							<span id="total_goods_price">7,128,000</span> &#x20a9;  <!-- 총 토탈 금액  {금액~} -->
						</td>
					</tr>
					</table>
				</div>
				...
				<div class="goods_buttons_area">
					<ul class="goods_buttons_section">
						<li>
							<ul class="basic_btn_area">
								<li><button type="button" id="buy_basket" class="btn_resp size_extra2 NpayNo"><span designElement="text" textIndex="107" >장바구니</span></button></li>
								<li><button type="button" id="buy_btn2" class="btn_resp size_extra2 color2 NpayNo"><span designElement="text" textIndex="109" >구매하기1</span></button></li>
							</ul>
						</li>
					</ul>
				</div>
	...
	<div id="goods_contents_quick" class="set_preload">
		...
		<!-- 상품문의 -->
		<div class="goods_information_contents goods_qna hide" id="goods_qna_frame_div"><!-- 상품문의 게시판 가져옴 -->
			<p>.. 상품 문의 서비스 준비 중입니다.</p> 
		</div>
